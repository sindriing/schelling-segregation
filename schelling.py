...
def happy_val(x, y, g):
    ...
    for agent in surrounding.flatten():
        if me == agent: goodNeighbours+=1
        elif agent == 0: goodNeighbours+=0.5
...
def switcheroo(x, y, grid):
    ...
    for i, j in get_neighbours_location(x, y):
        ...
        elif happy(i, j, grid) and not mehappy and grid[x, y] != grid[i, j] and grid[i, j] != 0:
...
def add_frame(arr, ims):
    temp = plt.imshow(arr[1:-1, 1:-1], animated=True, vmin=0.70, vmax=args.players+0.1)
    ims.append([temp])
...
def parse_arguments():
    ...
    parser.add_argument("-s", "--size", type=int, default=50, help="Size of the simulation. Default:80")
    parser.add_argument("-p", "--players", type=int, default=2, choices=[2, 3, 4], help="number of players in the model. default:2")
...
if __name__ == "__main__":
    ...
    if args.mode == "gif":
        ...
        ani = animation.ArtistAnimation(fig, imgrid, interval=100, blit=False, repeat_delay=5000)
